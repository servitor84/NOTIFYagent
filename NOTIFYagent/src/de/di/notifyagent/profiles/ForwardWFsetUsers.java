...
import de.di.notifyagent.jobs.ForwardWFsetUsersJob;
 ...
 * @author Rahman
 */
public class ForwardWFsetUsers extends BaseProfile {
...
//                workerProperties.setProperty(name, props.getProperty(name));
//            }
            if(name.equals("Interval") || name.equals("Type")) {
                this.workerProperties.setProperty(name, props.getProperty(name));
            }
        }
    }
    ...
    public Schedulable createJob(int id, Properties props) {
        return new ForwardWFsetUsersJob(id, props);
    }
}
